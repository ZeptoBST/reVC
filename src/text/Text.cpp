...
#endif
{
	CKeyEntry *found;
	char errstr[25];
	char textstr[25]; // ZAdd: For custom hardcoded text. #AutoSave
	int i;
	...
	else if(key == "FE_ASAV")	// ZAdd: AutoSave text. #AutoSave
	{
		sprintf(textstr, "Game saved", key);
		for(i = 0; i < 25; i++)
			WideErrorString[i] = textstr[i];
