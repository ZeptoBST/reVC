...
CAudioPedHit Listo[MAX_AUDIOPEDHIT]; // ZAdd: #Blood droplets

CAudioHydrant *CAudioHydrant::Get(int n) { return &List[n]; }

CAudioPedHit *CAudioPedHit::Get(int n) { return &Listo[n]; }	//ZAdd: #Blood droplets
...
CParticleObject::AddObject(uint16 type, CVector const &pos, CVector const &target, float size, uint32 lifeTime, RwRGBA const &color, uint8 remove)
{
	...
	switch ( type )
	{
		...
		case POBJECT_PED_HIT_BLOOD: //ZAdd: #Blood droplets
		{	
	...
	if ( !CGame::playingIntro )
	{
		if ( (this->GetPosition() - TheCamera.GetPosition()).MagnitudeSqr2D() > SQR(100.0f) )
		{
			if ( this->m_bRemove )
			{
				...
				if ( this->m_Type == POBJECT_PED_HIT_BLOOD )	//ZAdd: #Blood droplets
