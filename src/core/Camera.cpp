...
CCamera::Init(void)
{
	...
	if(!FrontEndMenuManager.m_bWantToRestart){
		...
		if(m_fMouseAccelHorzntl == 0) // ZAdd: Fixed mouse sensitivity being reset when starting a new game.
		{
			...
			if(CPad::GetPad(0)->CycleCameraModeUpJustDown() && !CReplay::IsPlayingBack() &&
			   ...
			   !m_WideScreenOn && !m_bFailedCullZoneTestPreviously && !m_bFirstPersonBeingUsed){
				...
				if(FrontEndMenuManager.m_ControlMethod == CONTROL_STANDARD){
					...
					else if(PedZoomIndicator == CAM_ZOOM_1) // ZAdd: Somehow it broke, or I never noticed. Either way it's fixed now. #Camera mode
						PedZoomIndicator = CAM_ZOOM_2;
					else
