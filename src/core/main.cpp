...
Idle(void *arg)
{
	...
	if(autoSave == 1) // ZAdd: AutoSave loop code. #AutoSave
	{
		CEntity *pEntityToTeleport = FindPlayerEntity();
		CPlayerPed *playerPed;
		playerPed = FindPlayerPed();
		CRunningScript neder{};

		if(CTheScripts::IsPlayerOnAMission() != 1 && !playerPed->bInVehicle) {
			int8 SaveSlot = PcSaveHelper.SaveSlot(7);           // ZAdd: Save to slot 8. #AutoSave
			PcSaveHelper.PopulateSlotInfo();                    // ZAdd: Also needed. #AutoSave
			CHud::SetHelpMessage(TheText.Get("FE_ASAV"), true); // ZAdd: Save confirmation message. #AutoSave
		}
	}
