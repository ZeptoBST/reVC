...
bool controlValue = 0;	// ZAdd: #Input switch
#endif
...
CPad::UpdatePads(void)
{
	...
	if(GetPad(0)->PCTempJoyState.CheckForInput()) {
		...
		controlValue = 1;                      // ZAdd: #Input switch
		CCamera::m_bUseMouse3rdPerson = false; // ZAdd: #Input switch
	} else {
#endif
	...
	if(GetPad(0)->PCTempKeyState.CheckForInput()) { // ZAdd: #Input switch
		IsAffectedByController = true;          // ZAdd: #Input switch
		controlValue = 0;                       // ZAdd: #Input switch
	}
	...
	if(IsAffectedByController && (GetPad(0)->PCTempKeyState.CheckForInput() || GetPad(0)->PCTempMouseState.CheckForInput())) {
		...
		CCamera::m_bUseMouse3rdPerson = true;	// ZAdd: #Input switch
	}
#endif
